<template>
    <game-layout title="Game" :showNavbar="false">
            ...
            <div class="modal">
                <label class="modal-box relative" for="">
                    ...
                    <div class="py-4 w-full flex flex-col justify-center text-center">
                        ...
                        <p class="mt-6">Please wait for the announcement of the winners. In the meantime, explore our tech wall and customize your own background. Keep an eye on the leaderboard as prizes will be awarded after the final talk. If you're in the top 10, visit the Ranium booth at 4 PM to collect your prize.</p>
                    </div>
                    <div class="modal-action flex-col justify-center mt-2 gap-2">
                        <div class="flex justify-center">
                            <Link class="btn btn-lg btn-primary text-xl" :href="route('leaderboard', {event: $page.props.currentEvent.slug})" v-if="!!$page.props?.appSettings?.show_leaderboard.value">Leaderboard</Link>
                        </div>
                        <div class="flex justify-center gap-2">
                            <button class="btn btn-outline btn-primary" @click="openAboutModal">About</button>
                            <button class="btn btn-outline btn-primary" @click="$inertia.visit(route('tech-wall'))">Tech Wall</button>
        ...
        handleKeydown(event, index) {
            ...
            if(keyCode == 229) {
                ...
                if(found?.length) {
                    ...
                    this.lastBackspaceIndex = null;
                }
                else {
                    this.$refs.guess[Math.max(index - 2, 0)]?.focus();
                    if (this.lastBackspaceIndex === index) {
                        this.$refs.guess[Math.max(index - 2, 0)]?.focus();
                        this.lastBackspaceIndex = null;
                    } else {
                        this.$refs.guess[index - 1]?.focus();
                        this.lastBackspaceIndex = index;
                    }
                }
            }
            else if(keyCode == 8) {
                this.lastBackspaceIndex = null;
                const oldVal = this.$refs.guess[index - 1].value;
                this.$refs.guess[index - 1].value = "";
                this.$nextTick(function() {
                    if(oldVal === '')
                        this.$refs.guess[Math.max(index - 2, 0)]?.focus();
                    else
                        this.$refs.guess[Math.max(index - 1, 0)]?.focus();
                });
            }
